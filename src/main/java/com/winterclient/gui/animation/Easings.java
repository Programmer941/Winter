...
public class Easings {
    ...
    public static float easeInOutElastic(float x) {
        if (x <= 0) return 0;
        if (x >= 1) return 1;
        return (float) (x < 0.5 ? -(Math.pow(2, 20 * x - 10) * Math.sin((20 * x - 11.125) * ((2 * Math.PI) / 4.5))) / 2 : (Math.pow(2, -20 * x + 10) * Math.sin((20 * x - 11.125) * ((2 * Math.PI) / 4.5))) / 2 + 1);
    }
    ...
    public static float easeOutBack(float x) {
        float c1 = 1.70158F;
        return (float) (1 + (c1 + 1) * Math.pow(x - 1, 3) + c1 * Math.pow(x - 1, 2));
    }
    ...
    public static float getEasingValue(float x, Easing easing) {
        ...
        switch (easing) {
            ...
            default:
                System.err.println("Unknown Easing type " + easing.name());
                return x;
        }
    }
}
